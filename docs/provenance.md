...
The active batches `CLINIPROOF_BALANCED_V4` and `CLINIPROOF_SEEDCASES_V3` were frozen as new VAL ranges because `CLINIPROOF_BALANCED_V3` and `CLINIPROOF_SEEDCASES_V2` were already immutable. Those earlier batches, `CLINIPROOF_BALANCED_V2`, `CLINIPROOF_SEEDCASES_V1`, and `CLINIPROOF_TAXONOMY_V1`, remain in the repository as historical provenance. `CLINIPROOF_TAXONOMY_V1` is not an active review set.
