...
This directory holds frozen resident-review study copies. Every batch is **machine-validated synthetic resident-review cases pending clinician validation**. Software checks terminology, structure, category eligibility, deterministic state transition, answer-key consistency, implemented isolation checks, and resident-export leak protections. These records are **not clinically validated** until residents complete review. Machine validation is **not** clinical validation.

Three dataset identities must stay distinct:

| Identity | Public IDs | Location | Role |
| --- | --- | --- | --- |
| **A. `RESIDENT_VALIDATION_V1`** | `VAL-001`–`VAL-024` | this folder | Existing historical-name freeze. Predates the canonical CliniProof taxonomy. Artifacts still use `omission`, `dose_mismatch`, `frequency_mismatch`, `incorrect_continuation`. Mapping to `f1_*` below is **retrospective documentation only**. |
| **B. `CLINIPROOF_TAXONOMY_V1`** | `VAL-201`–`VAL-224` (sequences 801–824) | [`cliniproof_v1/`](cliniproof_v1/) | **Current** post-taxonomy clinician-validation batch (24 cases: 20 error-bearing, 4 clean controls). First freeze generated with canonical `error_family` / `error_category`. Covers all **currently implemented** CliniProof error categories — **not** complete coverage of the full conceptual taxonomy. |
| **C. `legacy_pre_taxonomy`** | `VAL-025`–`VAL-096` | [`legacy_pre_taxonomy/`](legacy_pre_taxonomy/) | Provenance archive of pre-taxonomy `RESIDENT_VALIDATION_V2`–`V4`. **Not** current resident packets, **not** taxonomy-compliant, **not** Family 2 evidence. Do not move them back into this directory as current packets. |

`CLINIPROOF_TAXONOMY_V1` coverage (audited; details in [`cliniproof_v1/README.md`](cliniproof_v1/README.md)): Family 1 **11** (`f1_omission` 3, `f1_commission` 2, `f1_dose_mismatch` 2, `f1_frequency_mismatch` 2, `f1_route_mismatch` 1, `f1_therapeutic_substitution` 1); Family 2 implemented **9** (`f2_monitoring_not_arranged` 2, `f2_held_med_no_restart_plan` 2, `f2_insufficient_supply` 2, `f2_hospital_only_continued` 1, `f2_inpatient_substitution_not_reverted` 1, `f2_pending_decision_followup_missing` 1); clean controls **4**; `f2_coprescription_omitted` **0** (`not_yet_implementable`).

`RESIDENT_VALIDATION_V1` uses historical injector names. Those names are **not rewritten** to canonical CliniProof IDs. Mapping (documentation only, retrospective): `omission` → `f1_omission`, `dose_mismatch` → `f1_dose_mismatch`, `frequency_mismatch` → `f1_frequency_mismatch`, `incorrect_continuation` → `f1_commission` (MATCH commission, **not** Family 2 held-med/no-restart-plan).

`CLINIPROOF_TAXONOMY_V1` is a **new** prospective set with a new `batch_code` and new VAL IDs. Target `error_family` and `error_category` are specified in that plan **before** generation. Ineligible categories abort; they do not silently substitute another category. Do not freeze it with the default plan or into this folder — that would reuse or overwrite V1.

Pre-taxonomy `RESIDENT_VALIDATION_V2`–`V4` (`VAL-025`–`VAL-096`) were generated by the four-name pre-taxonomy injector (PR #4, commit `ba346834b7685b2e6ae13bee374995c9c118ccf3`): `omission`, `dose_mismatch`, `frequency_mismatch`, `incorrect_continuation`; no canonical `error_family`; no `f1_*` / `f2_*` IDs; unknown preferred categories could silently fall back to `omission`. Do not regenerate them in place. A new taxonomy-compliant set must use a new `batch_code` and new VAL IDs.
...
The remainder of this README is the investigator catalog for **`RESIDENT_VALIDATION_V1`**. The taxonomy batch catalog is [`cliniproof_v1/README.md`](cliniproof_v1/README.md). The V2–V4 archive is [`legacy_pre_taxonomy/README.md`](legacy_pre_taxonomy/README.md).
...
| Role | Existing historical-name freeze (predates canonical taxonomy) |
...
| Folder | Batch |
| --- | --- |
| this folder | **A.** `RESIDENT_VALIDATION_V1` (historical-name freeze; predates taxonomy) |
| [`cliniproof_v1/`](cliniproof_v1/) | **B.** `CLINIPROOF_TAXONOMY_V1` (current post-taxonomy clinician-validation batch) |
| [`legacy_pre_taxonomy/`](legacy_pre_taxonomy/) | **C.** `RESIDENT_VALIDATION_V2`–`V4` archive (provenance only; **not** a current study packet) |

---

## Who receives which file

**Residents** receive, from **one current study** folder only (`RESIDENT_VALIDATION_V1` in this folder, or `CLINIPROOF_TAXONOMY_V1` in [`cliniproof_v1/`](cliniproof_v1/)):

- that folder’s `resident_validation_cases.json`
- that folder’s `resident_review_worksheet.csv` (and optionally the schema)

Do not mix V1 JSON with CliniProof investigator files, or with the pre-taxonomy V2–V4 archive, in the same packet. Do not give residents [`legacy_pre_taxonomy/`](legacy_pre_taxonomy/) as a current study packet.
...
The implemented freeze pipeline is **not** “ask an LLM to invent a case and then find an error”:
...
freeze-validation-batch       per VAL assignment:
        │                       assessment/batch specification
        │                       scenario selected
        │                       canonical terminology from local ref_*
        │                       deterministic structured clean case
        │                       source-backed clinical rules
        │                       clean case machine-validated
        │                       requested category eligibility checked
        │                       exact requested error injected (or skipped)
        │                       hidden assessment / answer-key state recorded
        │                       post-injection category-aware validation
        │                       freeze under immutable VAL-* if audit passes
        ▼
...
6. Validates again (category-aware). Post-injection validation is expected to pass the implemented rule engine. On **`RESIDENT_VALIDATION_V1`** (and the archived pre-taxonomy V2–V4 copies), the planted signal is a Family 1 discharge-list mutation using historical names. On **`CLINIPROOF_TAXONOMY_V1`**, Family 1 remains a list-transition discrepancy; Family 2 cases may leave the discharge list intact and omit monitoring, a restart plan, supply, follow-up, or a similar companion action. Do not describe every error-bearing case as “exactly one medication-list discrepancy.”
...
The committed resident export contains no `CaseAnswerKey`, no `SYN-*` identifiers, no `TEST_*` identifiers, no `LEAK_MARKERS` (`app/services/validation_batch.py`), no planted-error category, no error-family field, no target-medication metadata, no `clean_expected_state`, no `injected_state`, and no correct-action answer. The worksheet retains empty rating/response fields.
...
- `LEAK_MARKERS` including `answer_key`, `error_family`, `error_category`, `clean_expected_state`, `injected_state`, `correct_action`, `trigger_meds`, `is_clean_control`, and related investigator fields
...
- **V1 planted errors** use four historical families only. Duplicate therapy, missing co-prescription, and Family 2 transition-of-care gaps are **not** in that freeze. Canonical Family 1 / Family 2 IDs are used on the separate `CLINIPROOF_TAXONOMY_V1` batch, which covers all currently implemented CliniProof error categories (`f2_coprescription_omitted` remains `not_yet_implementable` and is absent). Pre-taxonomy V2–V4 copies of the four-name mix are archived under [`legacy_pre_taxonomy/`](legacy_pre_taxonomy/) for provenance only; they are not current resident packets, not taxonomy-compliant, and not evidence of Family 2 implementation.
...
Reprint the taxonomy batch the same way, always with that batch’s `--plan` and `--output-dir` (see the table at the top of this file). Do not re-freeze the pre-taxonomy V2–V4 archive with the current injector.
