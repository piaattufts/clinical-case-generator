...
| Batch | Public IDs | Internal IDs | Master seed | Plan / exports | Injector names on disk | Role |
| --- | --- | --- | --- | --- | --- | --- |
| `RESIDENT_VALIDATION_V1` | `VAL-001`–`VAL-024` | `SYN-000101`–`SYN-000124` | `20260922` | this folder | historical: `omission`, `dose_mismatch`, `frequency_mismatch`, `incorrect_continuation` | Immutable original freeze |
| `CLINIPROOF_TAXONOMY_V1` | `VAL-201`–`VAL-224` | `SYN-000801`–`SYN-000824` | `20260922` | [`cliniproof_v1/`](cliniproof_v1/) | canonical `f1_*` / `f2_*` / `none` | Taxonomy-compliant study copy |

`RESIDENT_VALIDATION_V1` uses historical injector names. Those names are **not rewritten** to canonical CliniProof IDs. Mapping (documentation only): `omission` → `f1_omission`, `dose_mismatch` → `f1_dose_mismatch`, `frequency_mismatch` → `f1_frequency_mismatch`, `incorrect_continuation` → `f1_commission` (MATCH commission, **not** Family 2 held-med/no-restart-plan).

`CLINIPROOF_TAXONOMY_V1` is a **new** prospective set with a new `batch_code` and new VAL IDs. Target `error_family` and `error_category` are specified in that plan **before** generation. Do not freeze it with the default plan or into this folder — that would reuse or overwrite V1.

Pre-taxonomy `RESIDENT_VALIDATION_V2`–`V4` (`VAL-025`–`VAL-096`) are archived under [`legacy_pre_taxonomy/`](legacy_pre_taxonomy/). They were generated by the pre-taxonomy injector (PR #4, commit `ba346834b7685b2e6ae13bee374995c9c118ccf3`) and are **not** taxonomy-compliant batches. Do not regenerate them in place. A new taxonomy-compliant set must use a new `batch_code` and new VAL IDs.
...
| Folder | Batch |
| --- | --- |
| this folder | `RESIDENT_VALIDATION_V1` (immutable original freeze) |
| [`cliniproof_v1/`](cliniproof_v1/) | `CLINIPROOF_TAXONOMY_V1` (taxonomy-compliant) |
| [`legacy_pre_taxonomy/`](legacy_pre_taxonomy/) | `RESIDENT_VALIDATION_V2`–`V4` archive (pre-taxonomy; **not** a current study packet) |
...
Do not mix V1 JSON with CliniProof investigator files, or with the pre-taxonomy V2–V4 archive, in the same packet.
...
freeze-validation-batch       per VAL assignment:
        ...
        │                        Family 2 = missing companion action; V1 used historical Family 1 names)
