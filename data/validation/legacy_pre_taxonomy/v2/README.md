...
> **Legacy pre-taxonomy artifact — not a current resident packet, not a taxonomy-compliant study batch, and not evidence of Family 2 implementation.**
>
> Generated by the four-name pre-taxonomy injector on PR #4 (`cursor/three-more-validation-batches-9f03`, generating commit `ba346834b7685b2e6ae13bee374995c9c118ccf3`), which was closed without merge and is preserved as the historical record. Plans and answer keys use historical names only (`omission`, `dose_mismatch`, `frequency_mismatch`, `incorrect_continuation`) with no `error_family` and no `f1_*` / `f2_*` IDs. That injector silently substituted `omission` for an unknown preferred category.
>
> Do **not** treat this freeze as a CliniProof Family 1 / Family 2 batch. Do **not** regenerate it in place with the current taxonomy injector. Do **not** move these files back into `data/validation/` as a current study packet. A new taxonomy-compliant set must use a new `batch_code` and new VAL IDs (see [`../../cliniproof_v1/`](../../cliniproof_v1/)). Catalog: [`../README.md`](../README.md).
...
| [`resident_validation_cases.json`](resident_validation_cases.json) | Archived blinded export (not a current resident packet) | Dashboard-shaped cases. Do **not** hand this out as a current study set. |
