...
- `RESIDENT_VALIDATION_V1` (`VAL-001`–`VAL-024`) in this folder (historical injector names; immutable)
- `CLINIPROOF_TAXONOMY_V1` (`VAL-201`–`VAL-224`) in [`cliniproof_v1/`](data/validation/cliniproof_v1/) (canonical `f1_*` / `f2_*` / `none`)

Pre-taxonomy `RESIDENT_VALIDATION_V2`–`V4` (`VAL-025`–`VAL-096`) are archived under [`legacy_pre_taxonomy/`](data/validation/legacy_pre_taxonomy/). They were generated by the pre-taxonomy injector (PR #4, commit `ba346834b7685b2e6ae13bee374995c9c118ccf3`) and are **not** taxonomy-compliant study batches. Do not rewrite them to canonical IDs and do not regenerate them in place. A new taxonomy-compliant set must use a new `batch_code` and new VAL IDs.

Investigator catalogs describe planted errors — treat them as **investigator-only**. Do not rewrite V1 artifacts to canonical CliniProof IDs.
...
**This freeze is immutable.** Do not regenerate it “to match” the canonical taxonomy. A separate batch, `CLINIPROOF_TAXONOMY_V1` (`VAL-201`–`VAL-224`, sequences 801–824), uses the complete implemented CliniProof identifiers. Plan and exports: [`data/validation/cliniproof_v1/`](data/validation/cliniproof_v1/). Pre-taxonomy `RESIDENT_VALIDATION_V2`–`V4` (`VAL-025`–`VAL-096`) are archived under [`data/validation/legacy_pre_taxonomy/`](data/validation/legacy_pre_taxonomy/) and must not be regenerated in place.
