...
- successor of the immutable predecessors `CLINIPROOF_BALANCED_V3` (VAL-501–VAL-524) and `CLINIPROOF_BALANCED_V2` (VAL-301–VAL-324)
- pre-validation clinical QC applied in place: these cases have not been sent for human clinician ratings, so the batch code was not incremented for the chart-language and monitoring corrections described below
- controlled error injection after clean-case generation
- pending clinician validation

Links: [all cases](data/validation_balanced_v4/readable/all_cases.md) · [clinician packet](data/validation_balanced_v4/readable/clinician_validation_packet.md) · [worksheet](data/validation_balanced_v4/readable/clinical_validation_worksheet.csv) · [diversity report](data/validation_balanced_v4/diversity_report.md) · [investigator key](data/validation_balanced_v4/investigator_answer_key.md)

## 2. CLINIPROOF_SEEDCASES_V3
...
- successor of the immutable predecessors `CLINIPROOF_SEEDCASES_V2` (VAL-601–VAL-624) and `CLINIPROOF_SEEDCASES_V1` (VAL-401–VAL-424)
- pre-validation clinical QC applied in place, including a structured hold for the unresolved apixaban decision on VAL-823
- controlled error injection after clean-case generation
- pending clinician validation

Links: [all cases](data/validation_seedcases_v3/readable/all_cases.md) · [clinician packet](data/validation_seedcases_v3/readable/clinician_validation_packet.md) · [worksheet](data/validation_seedcases_v3/readable/clinical_validation_worksheet.csv) · [diversity report](data/validation_seedcases_v3/diversity_report.md) · [investigator key](data/validation_seedcases_v3/investigator_answer_key.md)

# Historical / preclinical-QC datasets

## CLINIPROOF_TAXONOMY_V1 (historical provenance)

- VAL-201–VAL-224, archived historical range
...
## CLINIPROOF_BALANCED_V3 and CLINIPROOF_SEEDCASES_V2

These freezes (VAL-501–VAL-524 in `data/validation_balanced_v3/` and VAL-601–VAL-624 in `data/validation_seedcases_v2/`) were the previous prospective revisions. They were already immutable when the final regimen QC was applied, so their case JSON was not edited. The current prospective sets are `CLINIPROOF_BALANCED_V4` and `CLINIPROOF_SEEDCASES_V3`. V3 and seed V2 are historical provenance and are **not** the current study set.

## CLINIPROOF_BALANCED_V2 and CLINIPROOF_SEEDCASES_V1

These merged freezes (VAL-301–VAL-324 and VAL-401–VAL-424) are **preclinical-QC versions**. They were not mutated in place. The corrected active revisions are `CLINIPROOF_BALANCED_V4` and `CLINIPROOF_SEEDCASES_V3`. They are **not** active prospective study sets.

## Where should I start?

The table below routes new reviewers to the two **active** datasets. Do not start with archived VAL-201–VAL-224, VAL-301–VAL-324, VAL-401–VAL-424, VAL-501–VAL-524, or VAL-601–VAL-624.

| Audience | Balanced set `CLINIPROOF_BALANCED_V4` | Seed-derived set `CLINIPROOF_SEEDCASES_V3` |
| --- | --- | --- |
| Resident / blinded reviewer | [balanced readable cases](data/validation_balanced_v4/readable/all_cases.md) | [seed readable cases](data/validation_seedcases_v3/readable/all_cases.md) |
| Clinician validator | [balanced clinician packet](data/validation_balanced_v4/readable/clinician_validation_packet.md) | [seed clinician packet](data/validation_seedcases_v3/readable/clinician_validation_packet.md) |
| Clinician entering ratings | [balanced worksheet](data/validation_balanced_v4/readable/clinical_validation_worksheet.csv) | [seed worksheet](data/validation_seedcases_v3/readable/clinical_validation_worksheet.csv) |
| Investigator | [balanced answer key](data/validation_balanced_v4/investigator_answer_key.md) | [seed answer key](data/validation_seedcases_v3/investigator_answer_key.md) |
| Diversity audit | [balanced diversity report](data/validation_balanced_v4/diversity_report.md) | [seed diversity report](data/validation_seedcases_v3/diversity_report.md) |
| Rubric | [balanced rubric](data/validation_balanced_v4/readable/validation_rubric.md) | [seed rubric](data/validation_seedcases_v3/readable/validation_rubric.md) |
...
The current active balanced batch is `CLINIPROOF_BALANCED_V4` (VAL-701–VAL-724). It supersedes `CLINIPROOF_BALANCED_V3` (VAL-501–VAL-524) and the earlier preclinical-QC freeze `CLINIPROOF_BALANCED_V2` (VAL-301–VAL-324). Those predecessors remain archived and were not mutated. `CLINIPROOF_BALANCED_V4` itself had not been sent for clinician ratings, so the final chart-language QC in this repository was applied to that same batch code rather than minting another version.
...
The current active seed-guided batch is `CLINIPROOF_SEEDCASES_V3` (VAL-801–VAL-824). It supersedes `CLINIPROOF_SEEDCASES_V2` (VAL-601–VAL-624) and the earlier preclinical-QC freeze `CLINIPROOF_SEEDCASES_V1` (VAL-401–VAL-424). Those predecessors remain archived and were not mutated. `CLINIPROOF_SEEDCASES_V3` itself had not been sent for clinician ratings, so the final chart-language QC, including the structured apixaban hold on VAL-823, was applied to that same batch code.
...
| Dimension | Balanced structured set | Resident-seed-guided set |
| --- | --- | --- |
| Starting point | predefined clinical scenario profiles | resident-authored clinical examples |
| Main design goal | controlled coverage and distributional balance | workflow realism from expert examples |
| Patient-level data | synthetic | synthetic |
| Exact source chart copied | N/A | No |
| Terminology | source-backed | source-backed |
| Medication regimen | curated profile logic | curated archetype and profile logic |
| Error assignment | predetermined | predetermined |
| Error injection | after clean-case validation | after clean-case validation |
| Diversity audit | clean-case fingerprint and similarity | within-archetype and across-archetype diversity |
| Prevalence weighted | No | No |
| Requires clinician validation | Yes | Yes |

The table compares design choices. It does not rank the strategies. Both produce synthetic patients, both resolve terminology from the same sources, both curate the administered regimen separately from the RxNorm product, and both inject at most one predetermined discrepancy after the clean case has been validated. The balanced set is built to cover named scenario profiles evenly. The seed-guided set is built so that discharge workflows resemble the structure of expert examples without copying those examples. A reader should choose a set because of the question being asked, not because one method is treated as the better generator.

### Source-backed terminology is not clinical truth

Terminology provenance validates concept identity: an RXCUI, LOINC code, or ICD-10-CM code on a case was retrieved from an official source rather than invented. RxNorm establishes medication concept identity. LOINC establishes laboratory and test identity. ICD-10-CM establishes coded diagnosis identity where a code is stored. UCUM-compatible units support unit consistency. Curated profile rules define the selected medication regimen: dose, route, frequency, indication, and temporal state. Synthetic numeric values are generated. Software rules catch only implemented constraints.
...
## Clinical regimen and medication-transition model

An RxNorm product and a clinical regimen are different objects. The product row identifies a concept, a strength, and a dose form. The regimen is the order a clinician would read: what was given, by which route, how often, why, and whether it continues after discharge. Copying product strength into the dose, or defaulting every drug to once daily, produced charts that looked coded but were not credible inpatient orders. The active generator therefore does not treat product strength as the administered dose and does not assume once daily.
...
The curated rows live in [`data/bootstrap/medication_regimens.json`](data/bootstrap/medication_regimens.json). Each row records the concept query, clinical role, dose, route, frequency, indication mapping, temporal role, profile restrictions, and a citation. Citations are investigator provenance. They are not printed into the resident chart as reassurance that the dose is “standard,” “correct,” or “not a universal dose.” If a real medication has no curated regimen, generation fails rather than inventing a dose. Test fixtures whose identifiers start with `TEST_` may still use a strength fallback so pipeline tests can run.

Temporal states used on active cases include:

- pre-existing home medication
- started during the hospitalization
- continued at discharge
- temporarily held, with a restart plan when that plan is part of the clean case
- hospital-only, stopped at discharge in the clean case
- discontinued
- formulary substitution, with home therapy resumed on the clean discharge list
- new at discharge
- pending restart or start decision, held when the unresolved question is whether to restart

Hospital-only drugs such as correctional insulin, prophylactic enoxaparin, and a proton-pump inhibitor started for an acute bleed are charted in ordinary clinical language. The resident note says what was done in the hospital. The investigator medication plan keeps `correct_discharge_state` and `decision_reason`, including the inpatient-only indication, and that reason is not copied into the resident indication. A pending restart, as on VAL-823, is a held discharge row plus a documented decision. It is not an active discharge prescription that also says the decision is undecided.

Older teaching snapshots `SYN-000901` through `SYN-000904` under [`data/docs/clinician_examples/`](data/docs/clinician_examples/) predate this regimen layer. Tables later in this README that quote those snapshots, including apixaban 2.5 MG once daily and a `1 tablet` fallback, describe that older demonstration. They are not the active VAL charts. Current prospective apixaban is 5 MG orally twice daily.
...
The archived freeze `CLINIPROOF_TAXONOMY_V1` (VAL-201–VAL-224) remains unchanged as historical provenance. The two **active** prospective sets are `CLINIPROOF_BALANCED_V4` and `CLINIPROOF_SEEDCASES_V3`. Earlier prospective freezes `CLINIPROOF_BALANCED_V3`, `CLINIPROOF_SEEDCASES_V2`, `CLINIPROOF_BALANCED_V2`, and `CLINIPROOF_SEEDCASES_V1` remain unchanged. Passing the diversity audit does not mean the cases are clinically validated. Human clinician review is still required.
...
Clinical validation uses a single review stage. One clinician reads the complete case and records C1–C5 in that same pass. There is no separate plausibility-only stage and no second consensus stage. Use the packets for the two active sets in [Where should I start?](#where-should-i-start). Clinician validation is not the later resident study. Resident study responses are what a resident writes after reading a blinded chart. Those responses do not replace C1–C5, and a clinician rating is not a resident answer.

### C1 — Clinical plausibility

C1 asks whether the hospitalization could occur as charted. Reviewers consider presentation and demographics, diagnosis fit, vital signs, laboratory values and units, the medication regimen, the hospital course, internal chart consistency, and discharge and follow-up. A distracting accident in any of those domains can make the case unsuitable even when the intended discrepancy is technically present.

### C2 — Intended assessment problem

C2 asks whether the predetermined discrepancy is actually present and matches the investigator answer key. Controls should contain no planted discrepancy. The category in the answer key is the target. A different real problem does not satisfy C2.

### C3 — Detectability

C3 asks whether an internal-medicine resident can identify the problem from resident-visible information and say what should change. The chart must contain the evidence. It must not announce the answer. Phrases that state the expected disposition, call a dose correct, or label a drug as the hospital-only medication that must be stopped fail this criterion even if they are clinically true.

### C4 — No unintended clinically meaningful problem

C4 asks whether a second clinically meaningful problem is also present. This is the isolation criterion. A wrong frequency, a dose that does not match the formulation, a route or form mismatch, an indication that does not fit the drug, an accidental omission or commission, a temporal contradiction, missing monitoring that is not the target, conflicting restart instructions, misleading supply, or an acute medication listed as a home medication can give the resident a second plausible answer. Laboratory monitoring and a clinic appointment are not the same task. An anticoagulation clinic visit can remain while the INR monitoring task is the intended gap. If missing INR monitoring is not the intended target, the clean chart needs a coherent INR plan. Baseline regimen problems can invalidate a case by creating an alternative explanation.

### C5 — Difficulty

C5 is an expert estimate of how hard the chart should be for the intended learner. It is advisory. It does not by itself exclude a case. Actual difficulty will later be estimated from resident performance, not from this rating.

### Reviewer recommendation

- **Accept.** The chart can be used for the assigned target, or as a control.
- **Revise.** The chart needs a specified correction before use. Do not silently edit a batch after clinician review has started. A revision after that point is a new freeze.
- **Exclude.** The chart should not be used, even if software checks passed.

C1, C2, C3, and C4 must be acceptable before a case is used against its answer key. The fuller worksheet instructions are in [`docs/clinical_validation.md`](docs/clinical_validation.md).

## Active prospective sets

See [Current prospective validation datasets](#current-prospective-validation-datasets). `CLINIPROOF_BALANCED_V4` and `CLINIPROOF_SEEDCASES_V3` are the current prospective outputs. They had not been sent for human clinician ratings, so the final pre-validation QC was applied to those batch codes. Archived `CLINIPROOF_TAXONOMY_V1`, `CLINIPROOF_BALANCED_V3`, `CLINIPROOF_BALANCED_V2`, `CLINIPROOF_SEEDCASES_V2`, and `CLINIPROOF_SEEDCASES_V1` files must not be rewritten. After clinician review of the active sets starts, further clinical corrections should be a new batch code rather than another in-place edit.

## Human-readable review materials

See [Human-readable clinician validation packets](#human-readable-clinician-validation-packets) and the starting-point table above. Active case pages are VAL-701–VAL-724 and VAL-801–VAL-824. Archived VAL-201–VAL-224, VAL-301–VAL-324, VAL-401–VAL-424, VAL-501–VAL-524, and VAL-601–VAL-624 remain available for provenance.
...
`freeze-validation-batch` hardcodes `use_openai=False`, so the committed `CLINIPROOF_BALANCED_V4` and `CLINIPROOF_SEEDCASES_V3` freezes, and the archived `CLINIPROOF_TAXONOMY_V1`, `CLINIPROOF_BALANCED_V2`, and `CLINIPROOF_SEEDCASES_V1` freezes, used template narrative only.
...
**Worked examples below are older educational demonstrations**, not members of the blinded resident-validation study. They were generated with an earlier code path, template admission wording (no OpenAI call), sequences **901–904**, and seed **20260926**. Snapshots: [`data/docs/clinician_examples/`](data/docs/clinician_examples/). Those JSON files still show the pre-regimen behavior in which some doses copied RxNorm strength and the fallback frequency was once daily. Active study cases for residents are VAL-701–VAL-724 and VAL-801–VAL-824, which use the curated regimen layer described in [Clinical regimen and medication-transition model](#clinical-regimen-and-medication-transition-model). This walkthrough does **not** say which `VAL-*` cases are controls or which discrepancy was planted. Do not treat apixaban 2.5 MG once daily, furosemide `1 tablet`, or a creatinine reported in µmol/L on these snapshots as the current prospective method.
...
Each medication is written in the contexts that match its temporal state. A continued home medication appears at home, in the hospital, and at discharge, at the curated dose, route, and frequency for that profile. A drug started during the admission has no home row. A hospital-only drug is active inpatient and absent from the clean discharge list, with investigator `decision_reason` recording the inpatient-only indication. A pending restart is held at discharge rather than left active. A `CaseMedicationPlan` row records the intended reconciliation decision. Dose and frequency come from the curated regimen, not from the RxNorm strength string and not from a once-daily default.

The heart-failure medication table in Worked Example 1 shows an **older** snapshot, `SYN-000901`, from before that regimen layer. Read it as a historical demonstration of list structure, not as the current dose policy.

This matters clinically because review is about transitions from home through hospital to discharge, not about whether a drug name is a valid RxNorm concept. The drug concept and RXCUI are source-backed. The administered dose, route, frequency, indication, and temporal state are curated. The assessment layer then counts mechanically detectable findings, including Family 1 discharge or plan mismatches and Family 2 missing companion actions. Artifacts in the older snapshots, such as an oral-solution ACE inhibitor, topical ibuprofen, or `1 tablet` of furosemide solution, are not the current prospective charts.
...
Clinical validation uses a single review stage. A clinician or resident reviews the complete frozen case once and completes C1–C5 in the same review. Ratings are recorded on the batch-specific worksheet (see [Where should I start?](#where-should-i-start)). Software does not fill those ratings. The review files are the clinician validation packets for `CLINIPROOF_BALANCED_V4` and `CLINIPROOF_SEEDCASES_V3`.

Separately, when the frozen cases are later administered as an assessment, residents who must find the planted problem themselves use the blinded JSON for the chosen active set — [`data/validation_balanced_v4/resident_validation_cases.json`](data/validation_balanced_v4/resident_validation_cases.json) or [`data/validation_seedcases_v3/resident_validation_cases.json`](data/validation_seedcases_v3/resident_validation_cases.json) — and the matching worksheet schema. That resident performance worksheet is not the clinician-validation form.
...
The following fields are stored for the older teaching snapshot `SYN-000904` by `_write_answer_key` using definitions in `app/models/cases.py`. The clinical category is medication omitted at discharge (`f1_omission`). The family is Family 1 (`family_1`). In that snapshot the trigger medication is apixaban 2.5 MG Oral Tablet, RXCUI `1364435`, charted once daily. That product and frequency predate the curated regimen layer and are not used on the active VAL cases. Current prospective apixaban is 5 MG orally twice daily. The rationale in the snapshot is that the correct discharge medication list includes this continued home medication and that it was intentionally omitted from the discharge list. The expected action is to restore the omitted continued discharge medication from the medication plan. The intentional change records clean state present, injected state absent, context discharge, and seed `20260926:904:HF_INPATIENT`. Severity (`severity_ncc_merp`) and a priori difficulty (`difficulty_a_priori`) are null because software does not score them. `SYN-000904` is not an active study case.
...
Study residents receive the blinded JSON and review worksheet for the **chosen active set**: [`data/validation_balanced_v4/resident_validation_cases.json`](data/validation_balanced_v4/resident_validation_cases.json) (`CLINIPROOF_BALANCED_V4`) or [`data/validation_seedcases_v3/resident_validation_cases.json`](data/validation_seedcases_v3/resident_validation_cases.json) (`CLINIPROOF_SEEDCASES_V3`). They do **not** receive the investigator key, freeze plan, manifest, coverage files, or batch README catalogs. Do not send archived [`data/validation/resident_validation_cases.json`](data/validation/resident_validation_cases.json) (`CLINIPROOF_TAXONOMY_V1`) as the current study packet.
...
The worksheet asks residents to record the following fields, defined in [`data/validation_balanced_v4/resident_review_schema.json`](data/validation_balanced_v4/resident_review_schema.json) (the seed-guided set has the same schema under [`data/validation_seedcases_v3/resident_review_schema.json`](data/validation_seedcases_v3/resident_review_schema.json)):
...
In addition to the resident JSON, freeze export writes the following investigator files **in that batch’s directory**. Active sets live in [`data/validation_balanced_v4/`](data/validation_balanced_v4/) and [`data/validation_seedcases_v3/`](data/validation_seedcases_v3/). Archived V1 files remain in [`data/validation/`](data/validation/) for provenance only. The table says what extra information each file contains. Residents should not receive these files.
...
| Dose on the older `SYN-000901` snapshot | Often copied from stored strength | metoprolol `37.5 MG` |
| Dose when that snapshot had empty strength | Synthetic fallback on the snapshot only | furosemide `1 tablet` |
| Frequency on the older snapshot | Constant `once daily` | not the current prospective policy |
...
| Active resident JSON (balanced) | `data/validation_balanced_v4/resident_validation_cases.json` (`CLINIPROOF_BALANCED_V4`) |
| Active resident JSON (seed) | `data/validation_seedcases_v3/resident_validation_cases.json` (`CLINIPROOF_SEEDCASES_V3`) |
| Archived resident JSON | `data/validation/resident_validation_cases.json` (`CLINIPROOF_TAXONOMY_V1`, historical) |
| Active investigator keys | `data/validation_balanced_v4/investigator_answer_key.json` and `data/validation_seedcases_v3/investigator_answer_key.json` (investigator-only) |
| Active readable clinician packets | `data/validation_balanced_v4/readable/` and `data/validation_seedcases_v3/readable/` |
...
To **use the committed resident-review files** without regenerating: give reviewers the active JSON under `data/validation_balanced_v4/` or `data/validation_seedcases_v3/` and the matching empty worksheet. Do not send investigator keys. Details: [How to give cases to residents](#18-how-to-give-cases-to-residents).
...
| `data/validation_balanced_v4` | Active `CLINIPROOF_BALANCED_V4` (VAL-701–VAL-724) |
| `data/validation_balanced/readable` | Preclinical-QC balanced clinician packets |
| `data/validation_balanced_v4/readable` | Active balanced clinician packets |
...
| `data/validation_seedcases_v3` | Active `CLINIPROOF_SEEDCASES_V3` (VAL-801–VAL-824) |
| `data/validation_seedcases/readable` | Preclinical-QC seed-guided clinician packets |
| `data/validation_seedcases_v3/readable` | Active seed-guided clinician packets |
...
(`1806884` is the RXCUI stored for lisinopril in the archived historical `CLINIPROOF_TAXONOMY_V1` freeze, not a code invented by this repo.)
...
- Administered dose, route, and frequency come from the curated regimen in `data/bootstrap/medication_regimens.json`, not from RxNorm strength and not from a once-daily default. Older `SYN-000901`–`SYN-000904` snapshots still show the previous strength-copy behavior.
...
- [`data/validation_balanced_v4/batch_plan.json`](data/validation_balanced_v4/batch_plan.json) (`CLINIPROOF_BALANCED_V4`, VAL-701–VAL-724, sequences 1401–1424, `balanced_structured`)
- [`data/validation_seedcases_v3/batch_plan.json`](data/validation_seedcases_v3/batch_plan.json) (`CLINIPROOF_SEEDCASES_V3`, VAL-801–VAL-824, sequences 1501–1524, `resident_seed_guided`)
...
### Prospective balanced set `CLINIPROOF_BALANCED_V4`

`CLINIPROOF_TAXONOMY_V1` remains immutable and archived. An **active** balanced structured set is `CLINIPROOF_BALANCED_V4` (VAL-701–VAL-724, sequences 1401–1424) in [`data/validation_balanced_v4/`](data/validation_balanced_v4/). Preclinical-QC `CLINIPROOF_BALANCED_V2` remains in [`data/validation_balanced/`](data/validation_balanced/). Each assignment names a distinct clinical profile so clean-case diversity is specified before error injection. Freeze with:

```bash
clinical-case-generator freeze-validation-batch --plan data/validation_balanced_v4/batch_plan.json
clinical-case-generator export-validation-batch --batch-code CLINIPROOF_BALANCED_V4 --output-dir data/validation_balanced_v4
```

Do not point those commands at `data/validation/`. The V1 hashes must remain unchanged.

### Active seed-guided set `CLINIPROOF_SEEDCASES_V3`

`CLINIPROOF_TAXONOMY_V1` is archived. `CLINIPROOF_BALANCED_V4` and `CLINIPROOF_SEEDCASES_V3` are the two active prospective sets. `CLINIPROOF_SEEDCASES_V3` (VAL-801–VAL-824, sequences 1501–1524) is generated with `generation_strategy=resident_seed_guided` from abstracted archetypes in [`data/seed_cases/`](data/seed_cases/). Resident-authored DOCX files are immutable design references; they are not copied into study charts. Freeze with:

```bash
clinical-case-generator freeze-validation-batch --plan data/validation_seedcases_v3/batch_plan.json
clinical-case-generator export-validation-batch --batch-code CLINIPROOF_SEEDCASES_V3 --output-dir data/validation_seedcases_v3
```
...
| Item | archived historical `CLINIPROOF_TAXONOMY_V1` |
| --- | --- |
| Public IDs | archived `VAL-201`–`VAL-224` |
...
- **Error-bearing archived historical `CLINIPROOF_TAXONOMY_V1`:** one canonical `f1_*` or `f2_*` category specified in that batch plan **before** generation. Family 2 cases may look like a complete discharge list while a required monitoring row, restart instruction, supply duration, hospital-only stop, substitution revert, or follow-up is missing. This README does not list which archived `VAL-201`–`VAL-224` ids received which category.
...
```text
{"batch_code":"CLINIPROOF_TAXONOMY_V1","master_seed":20260922,"frozen":["VAL-201",...],"reused":[...],"rejected":[...]}  # archived historical example
```
...
- Balanced structured: [`data/validation_balanced_v4/`](data/validation_balanced_v4/) (`CLINIPROOF_BALANCED_V4`, VAL-701–VAL-724)
- Resident-seed-guided: [`data/validation_seedcases_v3/`](data/validation_seedcases_v3/) (`CLINIPROOF_SEEDCASES_V3`, VAL-801–VAL-824)

Archived historical files remain in [`data/validation/`](data/validation/) (`CLINIPROOF_TAXONOMY_V1`, VAL-201–VAL-224) and are **not** the current study packet. Generated files under `data/exports/` are gitignored. The table below uses the balanced directory as the path example. The seed-guided batch has the same filenames under `data/validation_seedcases_v3/`. The table tells you which file a resident may receive and which files are investigator-only.

| File | Purpose | Who should see it | Blinded? | Contains answer key? | Give to residents? |
| --- | --- | --- | --- | --- | --- |
| [`resident_validation_cases.json`](data/validation_balanced_v4/resident_validation_cases.json) | Dashboard-shaped cases (`VAL-*` ids, nested `ClinicalCase`, child arrays). Omits `CaseAnswerKey`. Titles use `VAL-*`. | Residents | Yes | No | **Yes — this is the case file** |
| [`resident_review_worksheet.csv`](data/validation_balanced_v4/resident_review_worksheet.csv) | Empty capture rows, one per VAL ID | Residents / study staff | Yes (no answers) | No | **Yes** |
| [`resident_review_schema.json`](data/validation_balanced_v4/resident_review_schema.json) | Worksheet field definitions (ratings 1–5, identified error type, comments, …) | Study staff; optional for residents | Yes | No | Optional |
| [`investigator_answer_key.json`](data/validation_balanced_v4/investigator_answer_key.json) | Seeds, `SYN-*`, error category, affected RXCUI, clean expected state, snapshots | Investigators | No | **Yes** | **No** |
| [`investigator_answer_key.md`](data/validation_balanced_v4/investigator_answer_key.md) | Human-readable key | Investigators | No | **Yes** | **No** |
| [`validation_manifest.json`](data/validation_balanced_v4/validation_manifest.json) | Freeze metadata, source versions, control status | Investigators | No | Control/error status | **No** |
| [`batch_plan.json`](data/validation_balanced_v4/batch_plan.json) | Planned VAL IDs, scenarios, inject flags, `error_family`, `error_category` | Operators / investigators | No | Planned errors | **No** |
| [`coverage_report.md`](data/validation_balanced_v4/coverage_report.md) | Counts by scenario/error/terminology | Investigators | No | Mix summary | **No** |
| [`scenario_coverage_matrix.md`](data/validation_balanced_v4/scenario_coverage_matrix.md) | Resolved meds/labs/diagnoses per family | Investigators | No | Not per-case answers | **No** |
| [`README.md`](data/validation_balanced_v4/README.md) | Batch catalog | Investigators | No | Mix / provenance | **No** |
| [`readable/all_cases.md`](data/validation_balanced_v4/readable/all_cases.md) | Human-readable charts without intended-assessment issues | Optional supporting view | Yes | No | Not a required review stage |
| [`readable/clinician_validation_packet.md`](data/validation_balanced_v4/readable/clinician_validation_packet.md) | Readable cases + intended issues + C1–C5 | Clinician/resident validators | Packet | **Yes** | Primary one-stage review file |
| [`readable/validation_rubric.md`](data/validation_balanced_v4/readable/validation_rubric.md) | C1–C5 rubric | Medical educators / validators | Rubric only | No | Rubric may be shared; it has no per-case answers |
| [`readable/clinical_validation_worksheet.csv`](data/validation_balanced_v4/readable/clinical_validation_worksheet.csv) | One empty rating row per case | Clinical reviewers / study staff | Identifiers only | No | Record C1–C5 here |
...
- Active balanced: [`data/validation_balanced_v4/readable/`](data/validation_balanced_v4/readable/)
...
1. **Send / import for review:** the active blinded JSON for the chosen set — [`data/validation_balanced_v4/resident_validation_cases.json`](data/validation_balanced_v4/resident_validation_cases.json) (`VAL-701`–`VAL-724`) or [`data/validation_seedcases_v3/resident_validation_cases.json`](data/validation_seedcases_v3/resident_validation_cases.json) (`VAL-801`–`VAL-824`). Clinicians validating whether the cases themselves are suitable should use the matching clinician validation packet and complete C1–C5 in one pass. Residents who must find the planted problem themselves should receive the blinded JSON, not the clinician validation packet.
...
The committed JSON and Markdown under `data/validation_balanced_v4/` and `data/validation_seedcases_v3/` are the active sources of truth. Archived files under `data/validation/` remain historical provenance.
...
```bash
clinical-case-generator freeze-validation-batch --plan data/validation_balanced_v4/batch_plan.json
# reused should list VAL-701 … VAL-724; rejected should be []
clinical-case-generator export-validation-batch --batch-code CLINIPROOF_BALANCED_V4
clinical-case-generator freeze-validation-batch --plan data/validation_seedcases_v3/batch_plan.json
# reused should list VAL-801 … VAL-824; rejected should be []
clinical-case-generator export-validation-batch --batch-code CLINIPROOF_SEEDCASES_V3
```
...
| Frozen study id | `validation_batch_cases.validation_case_id` | archived example `VAL-201` |
...
The repository currently includes a PostgreSQL schema and Alembic migrations through `d4e8b17c6a91`; clients for RxNorm, credentialed LOINC, UCUM, ICD-10-CM, DailyMed, RxClass, NLM conditions, and NLM HPO; bounded bootstrap; three curated rules; five inpatient scenarios plus seed archetypes; deterministic generation with Family 1 and Family 2 injectors (see the taxonomy section); freeze and export of two active blinded VAL batches (`CLINIPROOF_BALANCED_V4` and `CLINIPROOF_SEEDCASES_V3`) with archived `CLINIPROOF_TAXONOMY_V1` retained for provenance; a local reference search API and `/health`; and optional OpenAI narrative wording that was not used on the committed freezes.
...
The archived freeze `CLINIPROOF_TAXONOMY_V1` has additional study-design limits that motivated the two active sets. That historical batch is not evenly distributed by scenario: thirteen of twenty-four cases use the heart-failure inpatient skeleton, four use hypertension, three use atrial fibrillation with anticoagulation, three use type 2 diabetes, and one uses community-acquired pneumonia. It is not a prevalence-weighted or representative sample of inpatient medicine. Several error categories occur only once, so that batch alone does not support stable category-specific psychometric estimates. The active batch `CLINIPROOF_BALANCED_V4` rebalances those families and requires distinct clean-case fingerprints, but it is still not a prevalence-weighted sample and still requires clinician review. The active batch `CLINIPROOF_SEEDCASES_V3` uses resident-authored seed archetypes rather than the five template families and also requires clinician review. Required companion medication omitted (`f2_coprescription_omitted`) is intentionally absent because the software does not yet have a sufficiently source-backed deterministic rule for that situation (`not_yet_implementable`).

Further limits apply to both active sets:

- The cases are synthetic charts, not extracts from an electronic health record.
- Supported scenarios and archetypes are a bounded teaching set, not the range of inpatient medicine.
- Neither batch is prevalence-weighted. The balanced grid is a design choice. The six resident-authored examples are design references, not evidence of disease, medication, or error prevalence.
- A source-backed terminology concept is not automatically a clinically appropriate choice for a particular synthetic patient. Passing automated validation does not establish clinical validity.
- Rule coverage is incomplete. Only implemented constraints are enforced.
- Medication regimens are curated for the supported profiles. They are not a universal prescribing engine.
...
- Clinician C1–C5 review is still required. The internal QC report means ready for that review, not clinically validated.
- C5 is advisory. Actual difficulty requires later resident performance.

Official source ranking on the older teaching snapshots could produce technically source-valid but clinically atypical formulations. Those snapshots are not the active study charts. Clinical plausibility of the active charts still requires physician review.
